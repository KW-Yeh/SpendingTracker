...
export const MonthlyBudgetChart = ({ data }: MonthlyBudgetChartProps) => {
  return (
    <div className="flex h-full w-full flex-col">
      <h3 className="mb-4 text-center text-lg font-medium text-gray-800">月度預算分配</h3>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
        {getMonthlyTotals(data).map((item) => (
          <div key={item.month} className="rounded-lg bg-gray-50 p-3 text-center">
            <p className="text-sm font-medium text-gray-700">{item.month}</p>
            <p className="text-lg font-semibold text-gray-800">${normalizeNumber(item.total)}</p>
          </div>
        ))}
      </div>
    </div>
...
const getMonthlyTotals = (data: MonthlyBudgetData[]): { month: string; total: number }[] => {
